...
import com.nanami.TouchManager;
...
public class LAppView implements AutoCloseable {
    ...
    public void initializeSprite() {
        ...
        LAppTextureManager.TextureInfo backgroundTexture =
                textureManager.createTextureFromPngFile(ResourcePath.ROOT.getPath() + ResourcePath.BACK_IMAGE.getPath());

        // 画像の中心座標
        float x = windowWidth * 0.5f;
        float y = windowHeight * 0.5f;

        // Maintain aspect ratio and cover the whole screen
        ...
        if (imageAspect > screenAspect) {
            ...
            fWidth = windowHeight * imageAspect;
        } else {
            fWidth = windowWidth;
            fHeight = windowWidth / imageAspect;
        }
